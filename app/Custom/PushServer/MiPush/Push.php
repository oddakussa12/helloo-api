...
class Push
{
    ...
    {
        $this->secret = 'your app secret';
        $this->package = 'your app packagename';
    ...
    public function send()
    {
        ...
        $message1->description($this->desc); // 通知栏的descption
        $message1->passThrough(0);  // 这是一条通知栏消息，如果需要透传，把这个参数设置成1,同时去掉title和descption两个参数
        $message1->payload($this->payload); // 携带的数据，点击后将会通过客户端的receiver中的onReceiveMessage方法传入。
        ...
        print_r($sender->multiSend($targetMessage,TargetedMessage::TARGET_TYPE_ALIAS)->getRaw());
    ...
    public function Send2()
    {
        ...
        $message2->title($this->title);
        $message2->description($this->desc);
        $message2->passThrough(0);
        $message2->payload($this->payload); // 对于预定义点击行为，payload会通过点击进入的界面的intent中的extra字段获取，而不会调用到onReceiveMessage方法。
       ...
       print_r($sender->multiSend($targetMessage2,TargetedMessage::TARGET_TYPE_ALIAS)->getRaw());
