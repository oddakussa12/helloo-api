...
class Follow
{
    ...
    public static function isRelationExists(Model $model, $relation, $target, $class = null)
    {
        ...
        $prefix= '';
        $relation == 'followings'?$prefix = 'followable':($relation == 'followers'?$prefix = 'user':$prefix = 'user');
        if ($model->relationLoaded($relation)) {
            return $model->{$relation}->where('common_follows.'.$prefix.'_id', head($target->ids))->isNotEmpty();
        }

        return $model->{$relation}($target->classname)->where('common_follows.'.$prefix.'_id', head($target->ids))->exists();
    }
